	...
	namespace genos{
		...
		template<int N> constexpr unsigned long long MASK		(){return (MASK<N-1>() * 0x100 + 0xFF);};
		template<> 		constexpr unsigned long long MASK<0>	(){return 0;};
				...
				*--__ptr = '0';
				...
				switch (__flags & ios_base::basefield) {
					case ios_base::oct:
					{
						...
						constexpr unsigned long long mask = MASK<sizeof(_Integer)>(); 
						unsigned long long temp(__x & mask);	
						for(; temp != 0; temp>>=3)
						{
							*--__ptr = '0' + (temp & 0b111);
						};
