...
#define __ARRAY_SPREAD_SECTION(array_nm, order_tag) \
	"data.array_spread." #array_nm order_tag 
...
#define __ARRAY_SPREAD_ADD_NAMED_ORDERED(array_nm, ptr_nm, order, ...) \
	__ARRAY_SPREAD_ENTRY_DEF(static decltypeof(array_nm[0]), array_nm, \
...
#define __ARRAY_SPREAD_SIZE_MARKER(array_nm, marker) \
	({ \
		extern decltypeof(array_nm) __ARRAY_SPREAD_PRIVATE(array_nm, marker); \
...
#define __array_foreach_nm(element, array, size, _ptr) \
	for (const decltypeof(element) *_ptr = (array), *_end = _ptr + (size); \
...
#define __array_range_foreach_nm(element, array_begin, array_end, _ptr, _end) \
	for (const decltypeof(element) *_ptr = (array_begin), *_end = (array_end); \
			(_ptr < _end) && (((element) = *_ptr) || 1); ++_ptr)
...
#define __array_nullterm_foreach_nm(element, array, _ptr) \
	for (const decltypeof(element) *_ptr = (array); ((element) = *_ptr); ++_ptr)
...
#define __array_range_foreach_ptr_nm(element_ptr, array_begin, array_end, \
		_ptr, _end) \
	for (decltypeof(element_ptr) _ptr = (array_begin), _end = (array_end); \
...
#define __array_spread_foreach_nm(element, array, size, _ptr)    \
	for (decltypeof(element) volatile const *_ptr = (array),         \
				*_end = _ptr + (size);                           \
			(_ptr < _end) && (((element) = *_ptr) || 1); ++_ptr)
...
#define __array_spread_foreach_ptr_nm(element_ptr, array_begin,           \
		array_end, _ptr, _end)                                            \
	for (decltypeof(*element_ptr) volatile const *_ptr = (array_begin),       \
				*_end = (array_end);                                      \
			(_ptr < _end) && ((element_ptr) = (decltypeof(element_ptr))_ptr); \
			++_ptr)
...
# define __ARRAY_SPREAD_ADD(array_nm, ...) \
	typedef decltypeof(array_nm[0]) __array_spread_element_placeholder

# undef  __ARRAY_SPREAD_ADD_NAMED
# define __ARRAY_SPREAD_ADD_NAMED(array_nm, ptr_nm, ...) \
	static decltypeof(array_nm[0]) volatile const ptr_nm[]
