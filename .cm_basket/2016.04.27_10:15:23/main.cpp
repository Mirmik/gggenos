...
void set_controller()
{
	...
	periodic_move_x.set_task(10000000, 100000, 100000, 50, 1000);
...
string speed_com(string str)
{
	uint32_t speed = hexcode_to_uint32(str.c_str());
	com_x_speed(speed);
	return string("speed");
};

string testlink_com(string)
{
	return string("Hello,World!");
};

string stop_com(string str)
{
	stop();
	return string("stop");
};

string testctr_com(string str)
{
	set_test_controller();
	return string("testctr");
};

string homectr_com(string str)
{
	set_home_gen_controller();
	return string("homectr");	
};

string scanctr_com(string str)
{
	set_controller();
	return string("scanctr");
};
...
void program()
{
	...
	//charbuf<14> mes;
	//mes = S2S.mbuild(3, "0000000A");

	//if(S2S.send(&mes) != 0) Serial6.print("ErrorSend");
	//else Serial6.print(mes.c_str());
...
/*
		...
		charbuf<2> ch_buf = uint_to_hexcode2(cs);
...
//	delay(500);
//	gpio_port_clr_pin(GPIOD,14);
//	gpio_port_clr_pin(GPIOB, 9);
//	delay(1);
//	gpio_port_set_pin(GPIOB, 9);
};
